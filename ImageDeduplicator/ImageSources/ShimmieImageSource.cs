...
namespace ImageDeduplicator.ImageSources {
    public class ShimmieImageSource : DatabaseImageSource {
        ...
        const String shimmieDeleteQuery = " delete from image_tags where image_id = @ID;" +
                " delete from image_views where image_id = @ID;" +
                " delete from comments where image_id = @ID;" +
                " delete from note_histories where image_id = @ID;" +
                " delete from note_request where image_id = @ID;" +
                " delete from notes where image_id = @ID;" +
                " delete from numeric_score_votes where image_id = @ID;" +
                " delete from pool_images where image_id = @ID;" +
                " delete from source_histories where image_id = @ID;" +
                " delete from tag_histories where image_id = @ID;" +
                " delete from user_favorites where image_id = @ID;" +
                " delete from images where id = @ID";

        long StartId = -1, EndId = -1;

        public ShimmieImageSource(String provider, String connectionString, String tags, String imagePath) : base(tags, provider, connectionString,
            "", shimmieDeleteQuery) {
            this.tags = tags;
